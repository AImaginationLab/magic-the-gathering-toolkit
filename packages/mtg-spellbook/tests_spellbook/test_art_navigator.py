...
# ArtistSelected Message Tests
class TestArtistSelectionFromArtNavigator:
    """Tests for artist selection from art navigator ('e' key)."""

    @pytest.mark.asyncio
    async def test_browse_artist_action_posts_message_in_focus_mode(
        self, sample_printings: list[PrintingInfo]
    ) -> None:
        """Test that browse_artist action posts ArtistSelected message in focus mode."""
        from typing import ClassVar

        from textual.app import ComposeResult
        from textual.containers import Vertical

        from mtg_spellbook.widgets.art_navigator.messages import ArtistSelected
        ...
        class TestApp(App[None]):
            ...
            def compose(self) -> ComposeResult:
                with Vertical(id="main"):
                    yield EnhancedArtNavigator(id_prefix="test", id="test-nav")
        ...
        async with TestApp().run_test() as pilot:
            nav = pilot.app.query_one("#test-nav", EnhancedArtNavigator)
            await nav.load_printings("Test Card", sample_printings)

            # Should be in focus mode by default
            assert nav.current_view == ViewMode.FOCUS

            # Trigger browse_artist action
            nav.action_browse_artist()
            await pilot.pause()

            # Should have posted ArtistSelected message
            assert len(TestApp.messages) == 1
            assert TestApp.messages[0].artist_name == "Artist One"
            assert TestApp.messages[0].card_name == "Test Card"

    @pytest.mark.asyncio
    async def test_browse_artist_action_posts_message_in_gallery_mode(
        self, sample_printings: list[PrintingInfo]
    ) -> None:
        """Test that browse_artist action posts ArtistSelected message in gallery mode."""
        from typing import ClassVar
    ...
    @pytest.mark.asyncio
    async def test_browse_artist_with_no_artist_shows_warning(
        self,
    ) -> None:
        """Test that browse_artist shows warning when no artist info available."""
        # Create printing without artist
        printing_no_artist = PrintingInfo(
            uuid="no-artist",
            set_code="test",
            artist=None,
        )
        ...
        async with App().run_test() as pilot:
            await pilot.app.mount(nav)
            await nav.load_printings("Test Card", [printing_no_artist])

            # Should not crash, and should show warning notification
            nav.action_browse_artist()
            await pilot.pause()

    @pytest.mark.asyncio
    async def test_e_key_binding_exists_on_enhanced_art_navigator(self) -> None:
        """Test that 'e' key binding exists on EnhancedArtNavigator."""
        from textual.binding import Binding

        # Check BINDINGS list for 'e' key (exact match, not substring)
        e_bindings = [
            b for b in EnhancedArtNavigator.BINDINGS if isinstance(b, Binding) and b.key == "e"
        ]
        assert len(e_bindings) == 1
        assert e_bindings[0].action == "browse_artist"

    @pytest.mark.asyncio
    async def test_focus_view_browse_artist_action(
        self, sample_printings: list[PrintingInfo]
    ) -> None:
        """Test that FocusView browse_artist action posts message."""
        from typing import ClassVar
        ...
        class TestApp(App[None]):
            ...
            def compose(self) -> ComposeResult:
                with Vertical(id="main"):
                    yield FocusView("Test Card", id="focus")
        ...
        TestApp.messages = []
        async with TestApp().run_test() as pilot:
            focus = pilot.app.query_one("#focus", FocusView)
            await focus.load_printings("Test Card", sample_printings)

            # Trigger browse_artist action on FocusView
            focus.action_browse_artist()
            await pilot.pause()
